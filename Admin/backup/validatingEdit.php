	...
	<form class="form-horizontal widget-content" role="form" action="TaskManagement.php" method="post">
			...
			<input type="hidden" name="date" value="<?php echo $date; ?>" > 
				<div class="row-fluid">
					...
					<div  class="span4">
						End Date :
						<div class="input-append date form_datetime">
							<input class="form-control from_date" id="dp6" name="adate" type="date" value="<?php echo $adate;?>" placeholder="End Date" style="width: 130px;">
							<span class="add-on"><i class="icon-th"></i></span>
						</div>
					</div>
				</div>
		...
		<div class="form-group">
			<div class="row-fluid">
				...
				<div  class="span4">
					Assigned To :
					<select id="assigned_to" name="assigned_to" style="width: 200px;">
						<?php 
							$query=mysql_query("select * from staff where status = 'Active'")or die(mysql_error());
					...
					<!--<input class="form-control" id="assigned_to" name="assigned_to" type="text" placeholder="Assigned To" value="<?php echo $assigned_to;?>">-->
				</div>
		
				<div  class="span4">
					Scheduled For :
					<select multiple id="scheduled_for" name="scheduled_for" style="width:200px;"required>
                        <option value="Weekly" <?php if($scheduled_for=="Weekend") echo "selected" ?> >Weekend</option>
                        <option value="Daily" <?php if($scheduled_for=="Daily") echo "selected" ?> >Daily</option>
                        <option value="Monthly" <?php if($scheduled_for=="Monthly") echo "selected" ?> >Monthly</option>
                        <option value="Quartly" <?php if($scheduled_for=="Quartly") echo "selected" ?> >Quartly</option>
                        <option value="Monday" <?php if($scheduled_for=="Monday") echo "selected" ?> >Monday</option>
						<option value="Tuesday" <?php if($scheduled_for=="Tuesday") echo "selected" ?> >Tuesday</option>
						<option value="Wednesday" <?php if($scheduled_for=="Wednesday") echo "selected" ?> >Wednesday</option>
						<option value="Thrusday" <?php if($scheduled_for=="Thrusday") echo "selected" ?> >Thrusday</option>
						<option value="Friday" <?php if($scheduled_for=="Friday") echo "selected" ?> >Friday</option>
						<option value="Saturday" <?php if($scheduled_for=="Saturday") echo "selected" ?> >Saturday</option>
						<option value="Sunday" <?php if($scheduled_for=="Sunday") echo "selected" ?> >Sunday</option>
                     </select>
					<!--<input class="form-control" id="scheduled_for" name="scheduled_for" type="text" placeholder="Scheduled For" value="<?php echo $scheduled_for;?>">-->
				</div>
		...
		<div class="form-group">
			<div class="row-fluid">
				...
				<div  class="span4">
					Efficiency :
					<input class="form-control" id="efficiency" name="efficiency" type="text" placeholder="Efficiency" 
					value="<?php echo $efficiency;?>" readonly>%
				</div>
			</div>
		</div>
		<br>
