            ...
            <table class="table table-bordered data-table table-scrollable">
              ...
              <tbody>
				...
				if($_GET['id'] != null && $_GET['tab']=="today"){
					...
					$query1=mysql_query("SELECT * FROM task WHERE sdate <='$todaydate' AND ddate >='$todaydate' AND assigned_to='$name' AND status='In Progress'")or die(mysql_error());
					...
					$staffid=$result['staffid'];
					$todaydate=date("Y-m-d");
					$query1=mysql_query("SELECT * FROM task WHERE WEEKOFYEAR(sdate)=WEEKOFYEAR('$todaydate') AND assigned_to='$name' AND status='In Progress' ")or die(mysql_error());
					...
					$staffid=$result['staffid'];
					$todaydate=date("Y-m-d");
					$query1=mysql_query("SELECT * FROM task WHERE ddate >='$todaydate' AND assigned_to='$name' AND status='In Progress'")or die(mysql_error());	
					...
					$staffid=$result['staffid'];
					$todaydate=date("Y-m-d");
					$query1=mysql_query("SELECT * FROM task WHERE assigned_to='$name' AND status='Completed'")or die(mysql_error());		
					...
					$staffid=$result['staffid'];
					$todaydate=date("Y-m-d");
					$query1=mysql_query("SELECT * FROM task WHERE assigned_to='$name' AND status='In Progress'")or die(mysql_error());
				}else{
				$query1=mysql_query("select * from task")or die(mysql_error());
				}
