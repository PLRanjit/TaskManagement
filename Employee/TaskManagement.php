...
<html lang="en">
<head>
<title>Task Mgmt Employee</title>
<meta charset="UTF-8" />
...
<!--Header-part-->
<div id="header">
  <h1><a href="dashboard.html">Task Mgmt Employee</a></h1>
</div>
<!--close-Header-part--> 
            ...
            <table class="table table-bordered data-table table-scrollable">
              ...
              <tbody>
				...
				if($_GET['id'] != null && $_GET['tab']=="today"){
					...
					$result=mysqli_fetch_array($query);
					$name=$result['employment_id'];
				$query1=mysqli_query($con ,"select * from task where assigned_to='$name' or assigned_by='$name'")or die(mysqli_error());
				}
				...
				<tr>
					...
					<td><center>
						<div class="btn-group btn-group-sm">
							...
							$query=mysqli_query($con ,"select * from staff where staffid='$log'")or die(mysqli_error());
							$result=mysqli_fetch_array($query);
							$name=$result['employment_id'];
							$ch=$row['assigned_to'];
							...
							<button name="submit" type="submit" value="edit" class="btn btn-info btn-sm" style="width:35px;" 
							
							<?php if($_GET['id'] != $_SESSION['staffid'] and $_GET['tab'] != null || ($ch!=$name)){ echo 'disabled'; } ?>><span class="icon icon-edit"></button>
							</form>
						</div>
						<!--<hr style="margin:3px;">
						<div class="btn-group btn-group-sm">
